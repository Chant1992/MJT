...
#define REFRESH_HEAD_HEIGHT 40.f
...
@interface GJGCRefreshFooterView ()
...
@implementation GJGCRefreshFooterView
...
{
    //设置静态字符串
    self.pullString = @"上拉加载列表数据";
    self.releaseString = @"释放开始刷新...";
...
- (void)setupSubViews
{
    ...
    self.activeView.frame = CGRectMake(floorf((REFRESH_HEAD_HEIGHT -20)/2+60), floorf((REFRESH_HEAD_HEIGHT-20)/2-2), 20, 20);
...
{
    self.frame = CGRectMake(self.gjcf_origin.x,table.contentSize.height,self.gjcf_width,self.gjcf_height);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.isLoading) {
        ...
        // 更改tableView被包围得头部边距
        scrollView.contentInset = UIEdgeInsetsMake(0, 0,REFRESH_HEAD_HEIGHT, 0);//头部包围距离为偏移距离
        
    } else if (self.isDragging && scrollView.contentOffset.y > scrollView.contentSize.height - scrollView.bounds.size.height + REFRESH_HEAD_HEIGHT) {
        
        // 更改箭头得方向
        [UIView beginAnimations:nil context:NULL];
        
        if (scrollView.contentOffset.y > scrollView.contentSize.height - scrollView.bounds.size.height + REFRESH_HEAD_HEIGHT) {
...
- (void)startLoadingForScrollView:(UIScrollView *)scrollView
{
    ...
    scrollView.contentInset = UIEdgeInsetsMake(0, 0,REFRESH_HEAD_HEIGHT, 0);//改变tableview头部被包围状态
//    UIEdgeInsets inset = scrollView.contentInset;
    ...
    //执行刷新方法
    if (self.delegate && [self.delegate respondsToSelector:@selector(refreshFooterViewDidTriggerLoadMore:)]) {
        [self.delegate refreshFooterViewDidTriggerLoadMore:self];
    }
}
